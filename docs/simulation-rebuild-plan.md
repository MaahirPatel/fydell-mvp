...
| 9 | Work Receipts: close the raw `/results/[token]` bypass | **bypass closed and guarded; receipt UX work not started** |
...
## Reconciled against the external audit

An independent audit of `f5883df` was checked against HEAD rather than assumed
to still apply. Its mechanical findings are closed: lint is at zero errors from
21, `tsx` is declared so no release command downloads an undeclared package, and
no user-facing surface still references the old support address.

Six structural findings were re-verified in source. Five still held. One,
"a unique defense set can be regenerated by deletion and recreation", is real in
code but currently unreachable in normal flow, because re-analysis returns early
once a complete run exists. That makes it latent rather than active: the moment
supersession is implemented in phase 6, re-analysis becomes reachable and the
delete cascade starts destroying candidate answers. It has to be fixed in the
same change as supersession, not after it.

Still open and confirmed at HEAD: analysis runs inline in the request rather
than as a durable job, opening the employer report can trigger analysis as a
side effect, a completed run is returned instead of superseded even though
`is_current` and `supersedes_run_id` exist in the schema and nothing in `src/`
references the latter, and the defense endpoint lets the caller choose whether
their own response is recorded as a candidate answer or a facilitator note.

The audit's remaining P0 items agree with the sequencing already in this plan,
with one exception that was actionable without a database and is described next.

## Phase 9, in part: the disclosure boundary

The audit's P0 item "remove public result sharing that bypasses candidate
controlled receipt permissions" needed no database, so it was done now.

`/results/[token]` resolved a plaintext `sim_sessions.share_token` and rendered
the complete analysis result to anyone holding the URL, with no field scoping,
no expiry, and no way for the candidate to withdraw it. The previous pass left
it readable behind a warning banner on the reasoning that existing links should
not break. That was the wrong trade. The banner told the reader the candidate
could not withdraw the link while continuing to show them the work.

The route now reads nothing. It does not touch its token, opens no client, and
renders no part of the attempt, so the URL is inert rather than merely
discouraged. It still exists, so a circulating link explains itself and points
at the Work Receipt instead of returning a bare 404.

Fixing it surfaced a second leak that the audit did not name.
`GET /api/sim/results/[sessionId]` returned `shareUrl`, built from the same
plaintext token, and that route authorizes any active member of the candidate's
organization. An employer could read the token straight out of the API response
and publish a permanent, unrevocable link to the candidate's full result. No UI
consumed the field, so removing it broke nothing.

`npm run test:disclosure` is a source-level guard rather than a request test,
because the property is about which code exists: a route that returns nothing
today because a query happened to miss is not the same as a route that cannot
return anything. It asserts the retired page references none of the session,
analysis, credential, or report symbols, that nothing anywhere queries or writes
a plaintext `share_token`, that the result API neither selects the token nor
returns a share URL while keeping its access check, and that `/record/[token]`
still resolves through hashed tokens with field scoping. Restoring the old page
fails nine of its checks.

The `share_token` column is deliberately left in place. Nulling it is data loss
and belongs with the retirement plan, where it can be verified against a real
database.
